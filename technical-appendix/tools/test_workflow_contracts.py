...
require(daily, r'actions/deploy-pages@v5', "daily success must deploy Pages directly")
if daily.count("actions/deploy-pages@v5") != 1:
...
if daily.index("actions/deploy-pages@v5") > daily.index("python pipeline/state_store.py push-published-db"):
    raise AssertionError("published comparison state must advance only after Pages deployment succeeds")
if daily.index("actions/deploy-pages@v5") > daily.index("git push"):
